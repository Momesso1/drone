...
class VisualizeGraphPublisher : public rclcpp::Node {
                ...
                while(toma <= maxToma)
                {
                    ...
                    if(toma <= maxSecurityDistance_)
                    {
                        ...
                        {   
                            ...
                            createdVertices.insert(index13);
...
public:
    ...
    {   
        ...
        parameterTimer = this->create_wall_timer(
            ...
            std::bind(&VisualizeGraphPublisher::check_parameters, this));
        ...
        timer_vertices_arbitrary = this->create_wall_timer(1000ms, std::bind(&VisualizeGraphPublisher::publish_obstacles_vertices, this));

        publisher_created_vertices = this->create_publisher<sensor_msgs::msg::PointCloud2>("/created_obstacles_vertices", 10);

    
        subscription1_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/rtabmap/cloud_map", 10, std::bind(&VisualizeGraphPublisher::callback_cloud_map, this, std::placeholders::_1));
        ...
        publisher121_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/received_cloud_map", 10);
         ...
         subscription2_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/rtabmap/odom", 10, std::bind(&VisualizeGraphPublisher::odomCallback, this, std::placeholders::_1));
...
int main(int argc, char * argv[])
{
    ...
    rclcpp::spin(std::make_shared<VisualizeGraphPublisher>());
