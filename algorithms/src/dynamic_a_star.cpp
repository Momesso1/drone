...
private:
    ...
    std::vector<std::tuple<float, float, float>> previousPath;
    ...
    void storeEdgesInPath(std::vector<std::tuple<float, float, float>>& originalPath) 
    {
        ...
        std::vector<std::tuple<float, float, float>> path = originalPath;
    ...
    */
    ...
    void callback_destinations(const geometry_msgs::msg::PoseArray::SharedPtr msg) 
    {
        ...
        if(!verticesDestino_.empty())
        {
            ...
            auto start_time_ = std::chrono::high_resolution_clock::now();
            bool obstacleFound = false, found = false;

            std::vector<std::pair<std::tuple<float, float, float>, std::tuple<float, float, float>>> pairs;
            ...
            std::tuple<float, float, float> start_tuple = std::make_tuple(array_inicial[0], array_inicial[1], array_inicial[2]);
            std::unordered_set<std::tuple<float, float, float>> set;
            float new_x = 0.0, new_y = 0.0, new_z = 0.0;
            ...
            if(!previousPath.empty())
            {
                ...
                for(int m = 1; m < previousPath.size() - 1; m++)
                {
                    ...
                    if(obstaclesVertices.find(previousPath[m]) != obstaclesVertices.end() && obstacleFound == false && set.find(previousPath[m]) == set.end() && m > 0)
                    {
                    ...
                    if(obstaclesVertices.find(previousPath[m]) == obstaclesVertices.end() && obstacleFound == true)
                    {
                        destination = previousPath[m];
    
                        std::pair<std::tuple<float, float, float>, std::tuple<float, float, float>> pair = std::make_pair(origin, destination);
                        pairs.push_back(pair);
                        std::cout << "TdesdadsOMA" << std::endl;
            ...
            if(previousPath.empty())
            {
                ...
                std::cout << "odsadassadpa" << std::endl;
            }
            else if(found == true) 
            {
                finalPath.clear(); 
...
public:
     ...
     : Node("a_star")
    {
        ...
        timer_visualize_path_ = this->create_wall_timer(1ms, std::bind(&AStar::publisher_dijkstra_path, this));
